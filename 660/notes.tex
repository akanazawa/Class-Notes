...
I(\phi)$, where $I(\phi) \approx 1/N\sum_n=1^NQ(x_n) = Q_{MC,N}(\phi)$ where $\{x_n\}$
...
Examples of idea to do this:
\begin{enumerate}
\item Antithetical variables: Assume $\phi \in [0,1]$, and $I(\phi)
...
+ O(\sig^2)$
So \textbf{the error is 1/4th the size} or $\sig^2$ instead of $\sig$.
\end{enumerate}
