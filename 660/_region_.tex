...
\message{ !name(notes2.tex) !offset(-3) }
...
\textbf{Problem 2}: 
\begin{enumerate}
\item QR factorization by gram-schimid
\item Making $r_{ik} = <\hat q_k, q_i>$ from $r_{ik} =<a_k, q_i>$,
...
\message{ !name(notes2.tex) !offset(-158) }
