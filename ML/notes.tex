...
For all of these mappings, $\phi(x)\cdot \phi(z)$ can be computed
...
\emph{Fromal representer theorem for perceptron}: At any point we can
...
Pro: for independent ensembles: can train them in parallel.
...
Now train classifiers on these $D_i$s and get $f_1,\dots, f_n$,
...
\begin{algorithmic}
  \STATE{Learn $f^1$ on $L^1$ and $f^2$ on $L^2$}
  \STATE{run $f^1$ and $f^2$ on $U$}
  \STATE{add unlabeled $\bar x_m$'s to $L$ only if $f^1(x^1_m) =
    f^2(x^2_m)$}
  \STATE{relearn $f^1, f^2$ on $L\cup \{\text{new examples}\}$}
\end{algorithmic}
...
\begin{algorithmic}
\FOR{$n=1,\dots, N$}
  \STATE{ choose $y_n \approx Bin(p)$}
  \FOR{ $d=1,\dots,D$ }
    \STATE{  choose $x_{n,d} \approx Bin(\theta_{y_n, d})$}
  \ENDFOR
  \STATE{ return $(x_n, y_n)$}  
\ENDFOR
\FOR{$m=1,\dots, M$}
  \STATE{ choose $y_m \approx Bin(p)$}
  \FOR{ $d=1,\dots,D$ }
  \STATE{  choose $x_{m,d} \approx Bin(\theta_{y_m, d})$}
    \ENDFOR
  \STATE{ return $(x_m)$}  
\ENDFOR
\end{algorithmic}
...
If we were doing only labeled, then the updates are : $\rho = \frac{\text{\# of positives}}{N}$, $\theta_{1,5} =
...
So the final update for $$\rho = \frac{\sum_iq(y_i=+1}{N+M}$$
...
For us we don't care that stationary distribution exists.
...
Alternative: \textbf{Particle Filtering}
\begin{itemize}
...
\subsection{Mini-viterbi algorithm} (mini because it's for MM)
...
\emph{Semi-Ring}: is a ring without the additive inverse.
...
THe graph is bi-partite so all neighbors of nodes will be
...
For others, the marginal probability is almost never exact and we
...
\subsubsection{Perceptron with multiple classes}
...
We're taking $\max_{\hat y}\vec w \cdot \phi(x,\hat y) + l(y_n, \hat
