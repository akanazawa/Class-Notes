...
\textbf{Definition:}A set $S$ is convex if $\forall x,y\in S, \ahx + (1-\ah)y \in S$ where
...
\emph{convex} if 
$$f(\ahx + (1-\ah)y) \le \ah f(x) + (1-\ah)f(y) \forall \ah \in
[0,1]$$
...
$l_p$ norm: $$||w||_p = (\sum_i|w_i|^p)^{1/p}, p\in[1,\intfy]$$
...
square. ($l_\infy(\vec w) = max \vec w$).
